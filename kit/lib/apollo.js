...
export function createClient(opt: Object = {}): ApolloClient {
...
export function getNetworkInterface(uri: string): any {
	...
	networkInterface.use(
		config.apolloMiddleware.map(f => ({ applyMiddleware: f }))
	);
	networkInterface.useAfter(
		config.apolloAfterware.map(f => ({ applyAfterware: f }))
	);
